...
class FlipDot_Controller_Class:
	...
	def updateDisplay(self, textMessage, column_offset=1, row_offset=0):
		...
		for segment in range(self.displaySegments):
			for segment_column in range(self.columns_per_Segment-1):
				for row in range(self.displayRows):
					if self.currentDisplayState[segment][segment_column][row] and not self.oldDisplayState[segment][segment_column-column_offset][row]:
				...
				self.clearRegisters()
				for row in range(self.displayRows):
					if not self.currentDisplayState[segment][segment_column][row] and self.oldDisplayState[segment][segment_column-column_offset][row]:
