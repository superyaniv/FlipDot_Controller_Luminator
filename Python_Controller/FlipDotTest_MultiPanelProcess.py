	...
	try:
		...
		while True:
			...
			for FlipDot_Panel in FlipDot_Panels:
				c=panelnum*5
				nMessage = message[c:]
				p = multiprocessing.Process(target=flipScroller, kwargs={'panelNumber':panelnum,'panelDisplay':nMessage,'columns_offset_total':columns_offset_total})
				proc.append(p)
				panelnum=panelnum+1
				logging.debug('starting %s', p.getName())
				p.start()
			...
			for p in proc:
				if p is main_process:
					continue
				p.join()
				logging.debug('joined %s', p.getName())
		...
		logging.debug('cancelling %s', p.getName())
		for p in proc:
			p.terminate()
		pass
